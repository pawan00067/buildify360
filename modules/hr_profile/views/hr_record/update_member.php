<div class="modal fade" id="appointmentModal">
                  ...
                  <div class="row">
                     ...
                     <div class="col-md-12">
                        ...
                        <div class="form-group" app-field-wrapper="staff_identifi">
                           ...
                           <input type="text" id="staff_identifi" name="staff_identifi" class="form-control" value="<?php echo new_html_entity_decode($hr_codes) ?>" aria-invalid="false" <?php if (!is_admin() && !has_permission('hrm_hr_records', '', 'edit') && !has_permission('hrm_hr_records', '', 'create')) {
                                                                                                                                                                                             echo 'disabled';
                                                                                                                                                                                          } ?>>
                        </div>
                     </div>
                  ...
                  <div class="row">
                     <div class="col-md-6">
                        ...
                        <div class="form-group" app-field-wrapper="email">
                           ...
                           <input type="email" id="email" name="email" class="form-control" autocomplete="off" value="<?php echo new_html_entity_decode($value) ?>" <?php if (!is_admin() && !has_permission('hrm_hr_records', '', 'edit') && !has_permission('hrm_hr_records', '', 'create')) {
                                                                                                                                                                        echo 'disabled';
                                                                                                                                                                     } ?>>
                        </div>
                     </div>
                           ...
                           <select name="workplace" class="selectpicker" id="workplace" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                              ...
                              <?php foreach ($workplace as $w) { ?>
                                 ...
                                 <option value="<?php echo new_html_entity_decode($w['id']); ?>" <?php if (isset($member) && $member->workplace == $w['id']) {
                                                                                                      echo 'selected';
                                                                                                   } ?>><?php echo new_html_entity_decode($w['name']); ?></option>
                           ...
                           <select name="job_position" class="selectpicker" id="job_position" data-width="100%" data-action-box="true" data-hide-disabled="true" data-live-search="true" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                              ...
                              <?php foreach ($positions as $p) { ?>
                                 <option value="<?php echo new_html_entity_decode($p['position_id']); ?>" <?php if (isset($member) && $member->job_position == $p['position_id']) {
                                                                                                               echo 'selected';
                                                                                                            } ?>><?php echo new_html_entity_decode($p['position_name']); ?></option>
                              <?php } ?>
                           </select>
                        </div>
                     </div>
                        ...
                        <?php if (has_permission('hrm_hr_records', '', 'edit') || has_permission('hrm_hr_records', '', 'create')) { ?>
                           ...
                           <!-- <?php
                                 ...
                                 if (!empty($get_team_manage)) {
                                    // Convert results to an array of IDs
                                    $selected_team = array_column($get_team_manage, 'team_manage_id');
                                 }

                                 echo render_select(
                                    'team_manage[]', // Name attribute (important for multi-select)
                                    $list_staff,
                                    ['staffid', 'full_name'],
                                    'hr_team_manage',
                                    $selected_team, // Preselect values
                                    ["multiple" => true, "data-actions-box" => true] // Enable multi-select
                                 );
                                 ?> -->
                           <?php
                           // Fetch selected staff IDs
                           $get_team_manage = get_team_manage($member->staffid);
                           $selected_team = !empty($get_team_manage) ? array_column($get_team_manage, 'team_manage_id') : [];
                           ?>

                           <div class="row">
                              <!-- Available Staff -->
                              <div class="col-md-5">
                                 <label><?php echo _l('Available Direct manager'); ?></label>
                                 <select id="available_staff" class="form-control selectpicker" data-live-search="true" multiple>
                                    <?php foreach ($list_staff as $staff) { ?>
                                       <?php if (!in_array($staff['staffid'], $selected_team)) { ?>
                              ...
                              <div class="col-md-2 text-center">
                                 <br><br>
                                 <button type="button" id="add_staff" class="btn btn-success btn-block">&gt;&gt;</button>
                                 <button type="button" id="remove_staff" class="btn btn-danger btn-block">&lt;&lt;</button>
                              </div>

                              <!-- Selected Team Managers -->
                              <div class="col-md-5">
                                 <label><?php echo _l('Selected Direct manager'); ?></label>
                                 <select id="selected_team_manage" name="team_manage[]" data-live-search="true" class="form-control selectpicker" multiple>
                                    <?php foreach ($list_staff as $staff) { ?>
                                       <?php if (in_array($staff['staffid'], $selected_team)) { ?>
                                          <option value="<?php echo $staff['staffid']; ?>" selected>
                                             <?php echo $staff['full_name']; ?>
                                          </option>
                                       <?php } ?>
                                    <?php } ?>
   ...
   <script>
      $(document).ready(function() {
         ...
         // Move selected staff from "Available Staff" to "Selected Team Managers"
         $('#add_staff').on('click', function() {
            $('#available_staff option:selected').each(function() {
               $(this).prop('selected', true).appendTo('#selected_team_manage'); // Keep selected
            });

            // Refresh SelectPicker UI
            $('#available_staff, #selected_team_manage').selectpicker('refresh');
         });

         // Move selected staff from "Selected Team Managers" back to "Available Staff"
         $('#remove_staff').on('click', function() {
            $('#selected_team_manage option:selected').each(function() {
               $(this).prop('selected', false).appendTo('#available_staff'); // Unselect when moved back
            });
