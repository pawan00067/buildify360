...
class Hr_profile_model extends App_Model
{
	...
	public function add_staff($data)
	{
		...
		if (isset($data['team_manage'])) {
			$team_manage = $data['team_manage'];
			unset($data['team_manage']);
		}
		...
		if ($staffid) {
			...
			if (isset($team_manage)) {
				foreach ($team_manage as $manager_id) {
					$this->db->insert(db_prefix() . 'staff_team_manage', [
						'staffid'      => $staffid,
						'team_manage_id' => $manager_id,
					]);
				}
			}
	...
	public function update_staff($data, $id)
	{
		...
		$staff_team_manage = $this->db->where('staffid', $id)->get(db_prefix() . 'staff_team_manage')->result_array();

		if (sizeof($staff_team_manage) > 0) {
			if (!isset($data['team_manage'])) {
				// If no team_manage data, remove all existing team managers
				$this->db->where('staffid', $id);
				$this->db->delete(db_prefix() . 'staff_team_manage');
			} else {
				foreach ($staff_team_manage as $team_manager) {
					if (isset($team_manage)) {
						if (!in_array($team_manager['team_manage_id'], $team_manage)) {
							// Remove team managers that are no longer assigned
							$this->db->where('staffid', $id);
							$this->db->where('team_manage_id', $team_manager['team_manage_id']);
							$this->db->delete(db_prefix() . 'staff_team_manage');
							if ($this->db->affected_rows() > 0) {
								$affectedRows++;
							}
			...
			if (isset($team_manage)) {
				foreach ($team_manage as $manager_id) {
					// Check if the team manager is already assigned
					$this->db->where('staffid', $id);
					$this->db->where('team_manage_id', $manager_id);
					$_exists = $this->db->get(db_prefix() . 'staff_team_manage')->row();
