...
<div class="col-md-12">
    <div class="table-responsive">
        <table class="table dpr-items-table items table-main-dpr-edit has-calculations no-mtop">
            ...
            <input type="hidden" name="action" value="wpc">
            <thead>
                <tr>
                    <th colspan="5" class="daily_report_title">Water Pollution Control Checklist</th>
                </tr>
                <tr>
                    ...
                    <th colspan="2" class="daily_report_head">
                        <span class="daily_report_label">Date: </span><input type="datetime-local" class="form-control" name="date" value="<?= isset($wpc_form->date) ? date('Y-m-d\TH:i', strtotime($wpc_form->date)) : '' ?>">
                    </th>
                </tr>
                <tr>
                    <th colspan="5" class="daily_report_head">
                        <span class="daily_report_label" style="display: ruby;">Location: <?php echo render_input('location', '', isset($wpc_form->location) ? $wpc_form->location : '', 'text', ['style' => 'width:150px;']); ?></span>
                    </th>
                </tr>
                <tr>
                    <th colspan="5" class="daily_report_head">
                        <span class="daily_report_label" style="display: ruby;">Inspected by: <?php echo render_select('inspected_by', get_staff_list(), array('staffid', 'name'), '', isset($wpc_form->inspected_by) ? $wpc_form->inspected_by : ''); ?></span>
                    ...
                    <tr class="main">
                        ...
                        <td>
                            <span class="daily_report_label" style="display: ruby;">
                                <?php echo render_select('items[' . $sr . '][status]', get_item_status_listing(), array('id', 'name'), '', isset($wpc_form_detail) ? $wpc_form_detail[$key]['status'] : ''); ?>
                            </span>
                        </td>
                        <td>
                            <span class="daily_report_label" style="display: ruby;">
                                <?php echo render_input('items[' . $sr . '][remarks]', '', isset($wpc_form_detail) ? $wpc_form_detail[$key]['remarks'] : '', 'text', ['style' => 'width:150px;']); ?>
                            </span>
                        </td>
                        <td>
                                    ...
                                    <div class="form-group">
                                        <div class="input-group">
                                            ...
                                            <span class="input-group-btn">
                                                <button class="btn btn-default add_more_attachments_wpc" data-item="<?= $sr ?>"
                            ...
                            <?php
                            if (isset($wpc_attachments) && count($wpc_attachments) > 0) {
                                foreach ($wpc_attachments as $attachment) {
                            ...
                            ?>
                                        <a href="<?php echo site_url('uploads/form_attachments/wpc_checklist/' . $form_id . '/' . $attachment['form_detail_id'] . '/' . $attachment['file_name']); ?>"
                                            class="display-block mbot5" download>
                                            <i class="<?php echo get_mime_class($attachment['filetype']); ?>"></i> <?php echo $attachment['file_name']; ?>
                                        </a>
                                        <a href="<?php echo admin_url('forms/delete_wpc_attachment/' . $attachment['id']); ?>"
...
<script type="text/javascript">
    ...
    $("body").on("click", ".add_more_attachments_wpc", function() {
        ...
        newAttachment
            .find("button")
            .removeClass("add_more_attachments_wpc")
    ...
    function resetAttachmentKeys() {
        ...
        $(".attachment_new").each(function() {
            const itemIndex = $(this).find(".add_more_attachments_wpc").data("item");
